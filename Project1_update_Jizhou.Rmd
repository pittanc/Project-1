---
title: "Exploring the Role of Age, Sex, and Weather in Marathon Performance"
author: "Jizhou Tian"
date: "2024-10-01"
abstract: >
 \textbf{Background:} Endurance exercise performance is well-known to be affected by environmental conditions. Previous research has shown that rising environmental temperatures negatively impact the performance, with more significant effects in longer-distance events such as marathons (Ely et al., 2007), and the extent of this impact may vary depending on both sex and age.  \par
 \textbf{Methods:} In this study, we analyzed data from five major marathon races to investigate the effects of weather on marathon performance across the lifespan in both men and women. We conducted an exploratory analysis aimed at three key objectives: examining how increasing age affects marathon performance in both men and women, exploring how environmental conditions influence performance and whether these effects vary by age and gender, and identifying which weather factors have the greatest impact on marathon outcomes. \par
 \textbf{Results and Conclusion:}  Our results indicate that age and sex are the most significant predictors of performance, with men generally performing faster than women across all age groups, and marathon times increasing with age, particularly after the age of 60. Among the environmental factors, WBGT (Wet Bulb Globe Temperature) had the most pronounced effect when not adjusting for other factors, as higher temperatures led to longer finishing times. When identifying the weather condition factor that has the most impact on marathon performance, a random forest model shows that solar radiation, relative humidity and AQI might be important variables. \par
output:
...
Endurance exercise performance is well-known to be affected by environmental conditions. Previous research has shown that rising environmental temperatures negatively impact performance, with more significant effects observed in longer-distance events such as marathons (Ely et al., 2007). These effects are less evident in female runners compared to males (Vihma, 2010). Furthermore, older runners, particularly masters men and women, exhibit greater sensitivity to higher temperatures and humidity in races like the New York City Marathon (Knechtle et al., 2021).

The impact of age on thermoregulation plays a crucial role in marathon performance. Older individuals have a reduced ability to tolerate heat stress, which may translate to decreased performance in hot environments (Kenney & Munce, 2003). However, the interaction of these factors with gender is still not fully understood. Men are more likely than women to slow over the course of a marathon, and older runners tend to display less pace variance compared to younger runners (Deaner et al., 2015; Nikolaidis, 2019).

In this study, we analyzed data from five major marathon races to investigate the effects of weather on marathon performance across the lifespan in both men and women. Specifically, we aimed to address three key objectives: examining how increasing age affects marathon performance in men and women, exploring the influence of environmental conditions on performance and whether these effects vary by age and gender, and identifying which weather factors, such as wet bulb globe temperature (WBGT), have the greatest impact on marathon outcomes.
...
setwd("D:/Brown/2024fall/Course/PHP2550 Practical Data Analysis/Portfolio/Project1")

aqi = read.csv("aqi_values.csv")
course_record = read.csv("course_record.csv")
marathon_dates = read.csv("marathon_dates.csv")
dat.orig = read.csv("project1.csv")
...
Since the missing rate (491/11564 = 4.2%) is small, the missing data can be considered negligible. We then drop all the missing cases and proceed with further analysis based on complete cases. After this step, we now have 
11073 observations.
...
As shown in the figure, we fit a smooth line for each gender, with the X-axis representing age in years and the Y-axis representing the best time in hours. The figure reveals a clear trend where marathon performance improves, reflected by a decrease in best time, up until the mid-20s for both men (slope:-0.16) and women (slope:-0.18). After reaching a peak around this age, performance gradually declines as age increases (slope:0.025).

For both men and women, the best times are observed between the ages of approximately 20 and 30. As age increases beyond 30, the rate of decline in performance becomes more significant, particularly after age 50, where we observe a sharper upward trend in best finishing time (slope=0.12).
...
We calculate the age-graded time based on the smoothing line of best time vs. age across sex. Fior each age-gender group, so the age-graded time is calculated by using the best time minus the best time predicted by the smoothing function. 

```{r}
age_sex_plot_smooth_data <- ggplot_build(age_sex_plot)$data[[1]]

dat %>% group_by(sex) %>% summarise(age.min=min(age),
                                    age.max=max(age))
age_values <- seq(15, 88, by = 1)

age_sex_timepred <- age_sex_plot_smooth_data %>%
  group_by(group) %>%  
  arrange(x) %>%  
  summarise(
    age = list(age_values),  
    besttime_pred = list(approx(x, y, xout = age_values)$y)
  ) %>%
  unnest(c(age, besttime_pred)) %>%
  mutate(sex=group-1) %>%
  mutate(sex=as.factor(sex)) %>%
  select(-group)

dat1 = inner_join(dat, age_sex_timepred, by=c("age", "sex")) %>%
  mutate(age_graded_time = besttime - besttime_pred)
```
...
WBGT_across_age_for_male_plot = 
  ...
  theme_minimal()
WBGT_across_age_for_male_plot
ggsave("../Plots_Project1/WBGT_across_age_for_male_plot.png", plot = WBGT_across_age_for_male_plot, width = 10, height = 8)
```
...
WBGT_across_age_for_female_plot = 
  ...
  theme_minimal()
WBGT_across_age_for_female_plot
ggsave("../Plots_Project1/WBGT_across_age_for_female_plot.png", plot = WBGT_across_age_for_female_plot, width = 10, height = 8)
...
AQI_across_age_for_male_plot
ggsave("../Plots_Project1/AQI_across_age_for_male_plot.png", plot = AQI_across_age_for_male_plot, width = 10, height = 8)
...
AQI_across_age_for_female_plot = 
  ...
  theme_minimal()
AQI_across_age_for_female_plot
ggsave("../Plots_Project1/AQI_across_age_for_female_plot.png", plot = AQI_across_age_for_female_plot, width = 10, height = 8)
...
wind_across_age_for_male_plot = 
  ...
  theme_minimal()
wind_across_age_for_male_plot
ggsave("../Plots_Project1/wind_across_age_for_male_plot.png", plot = wind_across_age_for_male_plot, width = 10, height = 8)
...
wind_across_age_for_female_plot = 
  ggplot(dat_female, aes(
  x = age,
  y = besttime,
  color = factor(wind_lt_10)
  ...
  theme_minimal()
wind_across_age_for_female_plot
ggsave("../Plots_Project1/wind_across_age_for_female_plot.png", plot = wind_across_age_for_female_plot, width = 10, height = 8)
...
The adjusted R-squared for Model 1 is 0.51, and the adjusted R-squared for Model 2 is 0.80. From the results table, we can learn that the coefficients for sex, age, WBGT and aqi are significant in both models. After we add the quadratic term of age into the model, the coefficient estimates change a lot. Thus, the form of the model has a large impact on our final results. It's hard to contain all the forms of variables and their different interactions in a single linear regression model. 

```{r echo=FALSE}
m1 = lm(
  besttime ~ sex + age + WBGT + rh + wind + aqi_mean +
    WBGT:sex + rh:sex + wind:sex + aqi_mean:sex +
    WBGT:age + rh:age + wind:age + aqi_mean:age,
  data = dat
)
kable(round(summary(m1)$coefficients, 4), caption = "Linear Regression Model 1")

m2 = lm(
  besttime ~ sex + age + WBGT + rh + wind + aqi_mean + I(age ^ 2) +
    WBGT:sex + rh:sex + wind:sex + aqi_mean:sex +
    WBGT:age + rh:age + wind:age + aqi_mean:age,
  data = dat
)
...
We decide to fit two random forest models instead. This is because random forest is able to consider all the forms of variables and their interactions. We can simply input all the predictors into the model, since random forest itself is able to choose among the predictors.

For the first random forest model, our outcome of interest is still Best finishing time (count in hours). We input dry bulb temperature, wet bulb temperature, black globe temperature, relative humidity, wind speed, AQI, solar radiation, dew point, WBGT, and WBGT flag into the model.

For the second random forest model, our outcome of interest is age-graded time. Our predictors do not include age anymore but include the same rest of the predictors.
...
```{r fig.cap = "Variable Importance Plot with best time", warning=FALSE, message=FALSE, echo=FALSE}
tree_imp = varImpPlot(rf_m, main="Variable Importance Plot")
```


```{r include=FALSE}
# Use age grading
rf_m_y = dat1[, "age_graded_time"]
rf_m_x = dat1[, c("sex",
              "Td",
              "Tw",
              "Tg",
...
```{r fig.cap = "Variable Importance Plot with age-graded time", warning=FALSE, message=FALSE, echo=FALSE}
tree_imp2 = varImpPlot(rf_m2, main="Variable Importance Plot (Age-graded)")
```


As shown in the variable importance plots, the left panel - %IncMSE (Percentage Increase in Mean Squared Error) - represents how much the model's mean squared error (MSE) increases when a particular variable is randomly permuted while others remain unchanged. The right panel - IncNodePurity (Increase in Node Purity) - shows the increase in node purity, which reflects how much a variable improves the homogeneity of the nodes and leaves in the trees of the Random Forest.

For the random forest model targeting the best time, we can learn that age and sex are the most important predictors in explaining the marathon performance, while other weather condition variables have small but similar contribution in explaining the marathon performance.

However, for the random forest model targeting the age-graded time, solar radiation, relative humidity and AQI are the most important in terms of mean decrease accuracy.
...
When identifying the weather condition factor that has the most impact on marathon performance, the form of variables and the interactions among them is very complicated. Adjusting for other factors, WBGT and AQI are significant predictors. Given all the weather condition factors we have, a random forest model where the best finishing time is the outcome tells us that the contribution in explaining the performance are similar among all the weather condition factors. A random forest model where the age-graded time is the outcome, tells that solar radiation, relative humidity and AQI are important variables. We use age-graded time because the continuous predictor age may split too many times in a random forest as it's a very important predictor. If we use age-graded time, the age will not be included in the predictors, then it becomes easier for us to find the potential pattern of other predictors.

One limitation of our study is the lack of detailed information on wind direction, which could provide further insights into the impact of wind on performance. Another limitation is the absence of data on participants who failed to complete the marathon. And the performance we have is only the top performance for each age-gender group. Our outcome of interest is the best finishing time in each age-gender group, which may be robust against changes in weather conditions, as these runners are already the best in their group and may not be easily affected by environmental factors.


## Conclusion

Overall, we conclude that both men and women reach their best marathon performance around their mid-20s. After reaching this peak, performance gradually declines with increasing age. While age and sex are the dominant factors influencing marathon performance, WBGT has a more significant effect on performance compared to other weather conditions, particularly in older participants, when not adjusting for other factors. A similar trend is observed in both males and females. When adjusting for other factors, WBGT and AQI are significant in linear models. A random forest model shows that solar radiation, relative humidity and AQI might be important variables.
